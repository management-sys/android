...
/* 카드 사용 현황 목록 아이템 */
@Composable
private fun CardUsageItem(cardInfo: TripDTO.CardUsagesInfo, name: String, onEvent: (TripAddEvent) -> Unit) {
    Card(
        modifier = Modifier.fillMaxWidth(),
        colors = CardDefaults.cardColors(containerColor = Color.White),
        ...
        Column(
            ...
            StartEndDateTimeEditItem(
                ...
                onStartChanged = { onEvent(TripAddEvent.ChangedCardDateWith(cardInfo.id, true, it)) },
                onEndChanged = { onEvent(TripAddEvent.ChangedCardDateWith(cardInfo.id, false, it)) }
            )
        }
    }
}
...
/* 카드 정보 목록 아이템 */
@Composable
private fun CardListItem(cardInfo: CardDTO.CardsInfo, isChecked: Boolean, onChecked: (Boolean) -> Unit) {
    Card(
        modifier = Modifier.fillMaxWidth(),
        colors = CardDefaults.cardColors(containerColor = Color.White),
        shape = RoundedCornerShape(10.dp),
...
/* 차량 정보 목록 아이템 */
@Composable
private fun CarListItem(carInfo: CarDTO.CarsInfo, isChecked: Boolean, onChecked: (Boolean) -> Unit) {
    Card(
        modifier = Modifier.fillMaxWidth(),
        colors = CardDefaults.cardColors(containerColor = Color.White),
        shape = RoundedCornerShape(10.dp),
